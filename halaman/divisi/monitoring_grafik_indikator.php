...
if ($id_indikator_aktif > 0) {
    ...
    // --- 4. Ambil Target Tahunan ---
    $q_t = "SELECT nilai_target FROM target WHERE id_indikator = $id_indikator_aktif AND id_cabang = $id_cabang AND tahun = $tahun_aktif AND bulan = 12";
    $res_t = mysqli_query($koneksi, $q_t);
    $t_row = mysqli_fetch_assoc($res_t);
    $target_tahunan = $t_row ? (float) $t_row['nilai_target'] : 0;

    // --- 5. Ambil Target Koreksi Bulanan ---
    $koreksi = [];
    $q_k = "SELECT bulan, nilai_target FROM target WHERE id_indikator = $id_indikator_aktif AND id_cabang = $id_cabang AND tahun = $tahun_aktif AND bulan IS NOT NULL";
    $res_k = mysqli_query($koneksi, $q_k);
    while ($k_row = mysqli_fetch_assoc($res_k)) {
        $koreksi[$k_row['bulan']] = (float) $k_row['nilai_target'];
    }

    // --- 6. Ambil Realisasi Bulanan ---
    $realisasi_bulanan = [];
    ...
    for ($m = 1; $m <= 12; $m++) {
        ...
        // --- 8. Data Realisasi Bulanan ---
        $r_m = isset($realisasi_bulanan[$m]) ? $realisasi_bulanan[$m] : 0;
        $chart_realisasi[] = round($r_m, 2);
    }

    // --- 9. Deteksi kondisi semua nol ---
    $semua_nol = true;
    foreach ($chart_target as $val) {
        if (abs((float)$val) > 0.001) { $semua_nol = false; break; }
    }
    if ($semua_nol) {
        foreach ($chart_realisasi as $val) {
            if (abs((float)$val) > 0.001) { $semua_nol = false; break; }
        }
    }
}
        ...
        <div class="main-content">
            ...
            <?php if ($id_indikator_aktif > 0): ?>
                <div class="kartu">
                    ...
                    <?php if (isset($semua_nol) && $semua_nol): ?>
                        <!-- Notifikasi khusus: semua target & realisasi = 0 -->
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 50px 30px; background: #f8fafc; border: 2px dashed #cbd5e0; border-radius: 12px; text-align: center; gap: 16px;">
                            <div style="background: #edf2f7; border-radius: 50%; width: 70px; height: 70px; display: flex; align-items: center; justify-content: center;">
                                <i class="fas fa-chart-bar" style="font-size: 2em; color: #a0aec0;"></i>
                            </div>
                            <div>
                                <p style="font-size: 1.05em; font-weight: 700; color: #4a5568; margin: 0 0 6px 0;">Belum Ada Data untuk Indikator Ini</p>
                                <p style="color: #718096; font-size: 0.9em; margin: 0; max-width: 400px;">
                                    Target dan realisasi untuk indikator <strong><?php echo htmlspecialchars($nama_indikator_aktif); ?></strong>
                                    pada tahun <strong><?php echo $tahun_aktif; ?></strong> seluruhnya bernilai <strong>0</strong>.
                                    Grafik tidak dapat ditampilkan karena belum ada data yang diisi.
                                </p>
                            </div>
                        </div>
                    <?php else: ?>
                ...
                <script>
                    const canvasEl = document.getElementById('indikatorChart');
                    if (canvasEl) {
                        ...
                        options: {
                                ...
                                tooltip: {
                                    ...
                                    callbacks: {
                                        label: function (context) {
                                            ...
                                            if (label) label += ': ';
                                            if (value !== null) {
                                                label += new Intl.NumberFormat('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
                                            }
                                            return label;
                                        },
                            ...
                            scales: {
                                y: {
                                    beginAtZero: true,
                                    grace: '15%',
                                    title: { display: true, text: 'Nilai (Format Lokal)', font: { size: 13, weight: 'bold' } }
                                },
                                x: {
                                    title: { display: true, text: 'Bulan (Periode <?php echo $tahun_aktif; ?>)', font: { size: 13, weight: 'bold' } }
                                }
                            },
                            interaction: {
                                mode: 'nearest',
