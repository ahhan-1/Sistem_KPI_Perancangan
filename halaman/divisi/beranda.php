...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <?php
            // --- 2. Notifikasi Laporan Baru ---
            $q_notif = "SELECT COUNT(*) as total FROM laporan_persetujuan WHERE status = 'dikirim'";
            $res_notif = mysqli_query($koneksi, $q_notif);
            $notif_data = mysqli_fetch_assoc($res_notif);
            if ($_SESSION['jabatan'] == 'Divisi' && $notif_data['total'] > 0):
            ?>
                <div id="notif-laporan" class="alert-notif" style="background: #ebf8ff; border-left: 5px solid #2b5797; padding: 15px 20px; border-radius: 8px; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); transition: opacity 0.5s ease;">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <div style="background: #2b5797; color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2em;">
                            <i class="fas fa-bell"></i>
                        </div>
                        <div>
                            <div style="font-weight: bold; color: #2b5797;">Pemberitahuan Laporan Baru</div>
                            <div style="font-size: 0.9em; color: #4a5568;">Ada <strong><?php echo $notif_data['total']; ?></strong> laporan kantor cabang yang menunggu review dan approval Anda.</div>
                        </div>
                    </div>
                    <a href="persetujuan_laporan.php" onclick="markNotifRead('laporan', <?php echo $notif_data['total']; ?>)" class="tombol" style="background: #2b5797; color: white; padding: 8px 20px; font-size: 0.85em; border-radius: 6px;">
                        Review Sekarang <i class="fas fa-arrow-right" style="margin-left: 5px;"></i>
                    </a>
                </div>
            <?php endif; ?>

            <?php
            // --- 3. Notifikasi Sanggahan Baru ---
            $q_dispute = "SELECT COUNT(*) as total FROM laporan_sanggahan WHERE status = 'dikirim'";
            $res_dispute = mysqli_query($koneksi, $q_dispute);
            $dispute_data = mysqli_fetch_assoc($res_dispute);
            if ($_SESSION['jabatan'] == 'Divisi' && $dispute_data['total'] > 0):
            ?>
                <div id="notif-sanggah" class="alert-notif" style="background: #fff5f5; border-left: 5px solid #e53e3e; padding: 15px 20px; border-radius: 8px; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); transition: opacity 0.5s ease;">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <div style="background: #e53e3e; color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2em;">
                            <i class="fas fa-exclamation-triangle"></i>
                        </div>
                        <div>
                            <div style="font-weight: bold; color: #e53e3e;">Laporan Ketidaksesuaian Data</div>
                            <div style="font-size: 0.9em; color: #4a5568;">Ada <strong><?php echo $dispute_data['total']; ?></strong> laporan dari cabang yang menyatakan data tidak sesuai.</div>
                        </div>
                    </div>
                    <a href="sanggah_cabang.php" onclick="markNotifRead('sanggah', <?php echo $dispute_data['total']; ?>)" class="tombol" style="background: #e53e3e; color: white; padding: 8px 20px; font-size: 0.85em; border-radius: 6px;">
                        Cek Laporan <i class="fas fa-arrow-right" style="margin-left: 5px;"></i>
                    </a>
                </div>
            <?php endif; ?>

            <script>
                // Fungsi untuk menandai notif sudah dilihat/diklik
                function markNotifRead(type, currentCount) {
                    sessionStorage.setItem('last_count_' + type, currentCount);
                }

                document.addEventListener('DOMContentLoaded', function() {
                    const types = [
                        { id: 'laporan', current: <?php echo $notif_data['total'] ?: 0; ?> },
                        { id: 'sanggah', current: <?php echo $dispute_data['total'] ?: 0; ?> }
                    ];

                    types.forEach(item => {
                        const el = document.getElementById('notif-' + item.id);
                        if (el) {
                            const lastCount = sessionStorage.getItem('last_count_' + item.id);
                            
                            // Jika jumlahnya masih sama dengan yang terakhir dilihat, sembunyikan
                            if (lastCount !== null && parseInt(lastCount) >= item.current) {
                                el.style.display = 'none';
                            } else {
                                // Tampilkan, lalu auto-hide setelah 3 detik
                                setTimeout(() => {
                                    if (el) {
                                        el.style.opacity = '0';
                                        setTimeout(() => {
                                            el.style.display = 'none';
                                            // Setelah auto-hide, anggap sudah "dilihat"
                                            markNotifRead(item.id, item.current);
                                        }, 500);
                                    }
                                }, 3000);
                            }
            ...
            <div class="kartu" style="padding: 0; background: transparent; border: none; box-shadow: none;">
                <?php if ($total_data > 0):
                    ...
                    // Middle ranks: 6 through (total - 5), excluding top 5 and bottom 5
                    $middle_start = 5;
                                ...
                                <tbody>
                                    <?php foreach ($top_5 as $idx => $r): ?>
                                        <tr class="top-rank">
                                            ...
                                            <td><div style="font-weight: bold; font-size: 0.9em;"><?php echo ucwords(strtolower($r['nama_cabang'])); ?></div></td>
